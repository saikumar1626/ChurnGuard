...
ChurnGuard is a full-stack machine learning application that predicts customer churn risk and explains *why* each customer is likely to leave, using SHAP (SHapley Additive exPlanations). It's built to mirror how retention teams at subscription businesses actually triage at-risk accounts — combining a live dashboard, single-customer prediction tool, batch CSV scoring, and full model performance reporting.
...
SHAP values are computed analytically for the linear model: `φᵢ = βᵢ(xᵢ − x̄ᵢ)`, where `x̄ᵢ` is the mean of feature `i` across the training set — making every explanation mathematically exact, not approximated.
...
| Single Predictor | Batch Analysis |
|---|---|
| ![Predictor](docs/screenshots/single-predictor.png) | ![Batch](docs/screenshots/batch-analysis.png) |

| Model Performance |
|---|
| ![Performance](docs/screenshots/model-performance.png) |
